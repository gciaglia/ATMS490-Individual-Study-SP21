...
%For all of Route 2
a = 1 ; 
z = length(lattFilteredR2) ; 
for k = (1:z) 
    Latt(a) = lattFilteredR2(k) ;
    Long(a) = longFilteredR2(k) ;
...
a = 1 ; 
z = length(lattFilteredR3) ; 
for k = (1:z) 
    Latt(a) = lattFilteredR3(k) ;
    Long(a) = longFilteredR3(k) ;
...
a = 1 ; 
z = length(lattFilteredR4) ; 
for k = (1:z) 
    Latt(a) = lattFilteredR4(k) ;
    Long(a) = longFilteredR4(k) ;
...
Ax = [ A2(:, 1), A3(:, 1), A4(:, 1) ] ; %normal vlaues to interp - averages
Ay = [ A2(:, 2), A3(:, 2), A4(:, 2) ] ;

%need to find max values and interpolate
% Max_values = [pnc1FilteredR2(1:375729), pnc1FilteredR3(1:375729), pnc1FilteredR4(1:375729) ] ;
% Max_ofAll = max(Max_values, [], 2) ;
